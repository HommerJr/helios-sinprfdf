...
function verify_ballot(election_raw_json, encrypted_vote_json, status_cb) {
    ...
    try {
        ...
        status_cb("O identificador da eleição é " + election_hash);
        ...
        if (election_hash == encrypted_vote.election_hash) {
            status_cb("A identificação da eleição confere com a cédula");
        } else {
            overall_result = false;
            status_cb("PROBLEMA = a identificação da eleição não confere com a cédula");
        }
