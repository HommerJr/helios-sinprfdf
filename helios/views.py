...
def one_election_edit(request, election):
    ...
    RELEVANT_FIELDS = ['short_name', 'name', 'description', 'use_voter_aliases', 'election_type', 'private_p',
                       'help_email', 'randomize_answer_order', 'voting_starts_at', 'voting_ends_at']
...
def test_cookie_2(request):
    ...
    if not request.session.test_cookie_worked():
        return HttpResponseRedirect(settings.SECURE_URL_HOST + (
                    "%s?%s" % (reverse(url_names.COOKIE_NO), urlencode({'continue_url': continue_url}))))
...
def new_trustee(request, election):
    if request.method == "GET":
        ...
        trustee.save()
        return HttpResponseRedirect(
            settings.SECURE_URL_HOST + reverse(url_names.election.ELECTION_TRUSTEES_VIEW, args=[election.uuid]))
    ...
    if election:
        ...
        if trustee:
            if trustee.secret == trustee_secret:
                ...
                return HttpResponseRedirect(settings.SECURE_URL_HOST + reverse(url_names.election.ELECTION_TRUSTEE_HOME,
                                                                               args=[election.uuid, trustee.uuid]))
...
def trustee_send_url(request, election, trustee_uuid):
    ...
    url = settings.SECURE_URL_HOST + reverse(url_names.TRUSTEE_LOGIN,
                                             args=[election.short_name, trustee.email, trustee.secret])
...
""" % (election.name, url)
    ...
    utils.send_email(settings.SERVER_EMAIL, ["%s <%s>" % (trustee.name, trustee.email)],
                     'Sua página de apuração para %s' % election.name, body)
...
def trustee_upload_pk(request, election, trustee):
    if request.method == "POST":
        ...
        try:
            election.admin.send_message("%s - sua chave de apurador foi carregada" % election.name,
                                        "Apurador %s (%s) carregou uma chave de apuração." % (
...
def one_election_cast_confirm(request, election):
    ...
    if request.method == "GET":
        ...
        if voter and voter.can_update_status():
            ...
            status_update_message = "I voted in %s - my ballot tracker is %s.. #heliosvoting" % (
...
def one_election_bboard(request, election):
    ...
    return render_template(request, 'election_bboard',
                           {'election': election, 'voters': voters, 'next_after': next_after,
...
""" % (election.name)
            ...
            # log it
            election.append_log(
                "Voter %s/%s and their vote were removed after election frozen" % (voter.voter_type, voter.voter_id))
...
def one_election_copy(request, election):
    ...
    return HttpResponseRedirect(
        settings.SECURE_URL_HOST + reverse(url_names.election.ELECTION_VIEW, args=[new_election.uuid]))
...
def one_election_compute_tally(request, election):
    ...
    if not _check_election_tally_type(election):
        return HttpResponseRedirect(
            settings.SECURE_URL_HOST + reverse(url_names.election.ELECTION_VIEW, args=[election.election_id]))
...
def release_result(request, election):
    ...
    if request.method == "POST":
        ...
        if request.POST.get('send_email', ''):
            ...
            settings.SECURE_URL_HOST + reverse(voters_email, args=[election.uuid]), urlencode({'template': 'result'})))
        else:
            return HttpResponseRedirect(
                settings.SECURE_URL_HOST + reverse(url_names.election.ELECTION_VIEW, args=[election.uuid]))
    ...
    if request.method == "POST":
        if bool(request.POST.get('confirm_p', 0)):
            ...
            if 'voters_file' in request.FILES:
                ...
                urlencode({'e': 'no voter file specified, try again'})))
...
def voters_email(request, election):
    ...
    if request.method == "GET":
        ...
        if email_form.is_valid():
            ...
            if voter:
                tasks.single_voter_email.delay(voter_uuid=voter.uuid, subject_template=subject_template,
                                               body_template=body_template, extra_vars=extra_vars)
            else:
                ...
                tasks.voters_email.delay(election_id=election.id, subject_template=subject_template,
                                         body_template=body_template, extra_vars=extra_vars,
