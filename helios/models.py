...
class Election(HeliosModel):
    ...
    def last_alias_num(self):
        ...
        return utils.one_val_raw_sql(
            "select max(cast(substr(alias, 2) as integer)) from " + Voter._meta.db_table + " where election_id = %s",
            [self.id]) or 0
    ...
    def process(self):
        ...
        for voter in voters:
            if voter['voter_type'] == 'password':
                ...
                user, _ = User.objects.get_or_create(user_type=voter['voter_type'], user_id=voter['voter_id'],
                                                     defaults={'name': voter['voter_id'], 'info': {}, 'token': None})
...
class Voter(HeliosModel):
    ...
    def generate_password(self, length=10):
        ...
        self.voter_password = utils.random_string(length,
...
class CastVote(HeliosModel):
    ...
    def issues(self, election):
        ...
        if self.vote.election_uuid != election.uuid:
            issues.append(
                "o UUID da eleição do voto não corresponde à eleição para a qual este voto está sendo emitido")
